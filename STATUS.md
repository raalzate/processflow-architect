...
| Self-test del arnés | `node scripts/harness-selftest.mjs` | verde — 7 hooks, 32 frenos probados (incluye DEPSHOOK, TOKENS, SVGFILL, BOTONMUDO, PLATAFORMA, ENRUTADO, RELEASEJOB y «no escribe temporales en `src/`»), el hook `commit-msg` en un repo git temporal, 11 casos de ruteo |
