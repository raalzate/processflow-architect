...
- **Versión publicada:** 0.8.4 — publicada el 2026-09-01 (`gh release edit v0.8.4 --draft=false`), con los 8 artefactos (tres instaladores, `latest*.yml`, blockmaps) y como `latest` de la API; `latest.yml` responde 200 por la URL que consulta el updater. Arregla #227: el menú «Diseño» declaraba `CmdOrCtrl+V` y se comía el pegar del sistema en TODO input de la app. `v0.8.3` quedó **en borrador y superada**: nunca se publicó, así que quien tenga 0.8.2 salta directo a 0.8.4. `v0.8.0`, `v0.8.1` y `v0.8.2` siguen publicadas; `v0.7.0` sigue en borrador
- **Versión en el repo:** 0.8.4 — igual a la publicada (PR #229). Es la primera versión que llega POR el botón «Actualizar» (T9 · #217): 0.8.2 estrenó los metadatos del updater y 0.8.4 es la que ese botón puede ofrecer
