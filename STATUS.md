...
- **Fecha del último gate completo:** 2026-09-21
- **Rama:** `fix/372-updater-autoupdater-undefined` (sale a `main` por PR)
- **Veredicto:** VERDE (`npm run gate`)
- **Versión publicada:** 0.11.0 — publicada el 2026-09-21 y `latest` de la API; trae el agente constructor (#308 · #332), los adjuntos por elemento (feature 016, #360), la notación entidad-relación y la paleta de formas generales. **Tiene el bug #372**: en Windows y Linux el botón de actualizar muere con `Cannot set properties of undefined (setting 'autoDownload')`, así que quien esté ahí debe bajar la 0.11.1 a mano. 0.9.0 quedó en borrador con sus 8 artefactos, recreada el 2026-09-21; `v0.7.0` se borró (superada por la 0.8.x)
- **Versión en el repo:** 0.11.1 — arregla #372: `electron-updater` es CJS y publica `autoUpdater` con un getter que cjs-module-lexer no detecta al cargarlo por `import()`, así que el nombrado llegaba `undefined`. Sólo pasaba empaquetado Y en Windows/Linux, y el mock de la suite inventaba un export nombrado que en producción no existe. Notas en `docs/releases/0.11.1.md`
- **Cierres verificados el 2026-09-02:** #239 (el agente lee la ficha de la caja: `read_element`, la marca `{spec,props,desc+}` en el digest, `set_element_spec { merge: true }` y un `review_specs` que marca criterios sin número y requisitos con tecnología) y la **feature 012 completa** (#240 con sus 10 tareas #241–#250): el documento fuente viaja dentro del proyecto y la cita de una caja resuelve a su fragmento. Gate completo verde en local y en CI; mergeado en `main` por el PR #251 (e52c593). El tracker quedó **sin issues abiertos**
- **Cierres verificados el 2026-09-01:** #227 (pegar con ⌘V: el menú «Diseño» ya no declara aceleradores de edición nativa; prueba en `designer-actions.test.ts`), #231 (aviso de actualización en el pie + menú Ayuda + descarga del `.dmg` en macOS), #235 (el `.exe` de Windows se llama como dice `latest.yml`: `release-updater-check` verde en las tres plataformas) y con ellos **la feature 011 completa** (#208 y su última tarea #217). El tracker quedó **sin issues abiertos**
- **Sin verificar:** que un proyecto con documentos fuente **abra en OTRA máquina** mostrando los mismos fragmentos (SC-004 de #240): lo que sí se mide es el ida y vuelta del modelo —serializar y deserializar sin perder el texto—, no dos equipos reales. Que la **ficha muestre el fragmento en la app viva** tras adjuntar por MCP (el componente `FuenteCitada`; la decisión sí está probada en `source-docs.ts`). Que el agente **use** `read_element` y `read_source` con el motor **local** en una conversación real: el ruteo y las herramientas están probados, la elección del modelo no la mide nadie. El **ciclo de actualización corriendo en Windows** (SC-001 de #208): instalar 0.8.6 ahí, esperar la siguiente y ver que se aplique sin salir de la app. Los metadatos ya se miden por comando (`npm run release:updater-check`), pero que la instalación se aplique en la máquina no lo mide nadie desde macOS — y hasta 0.8.6 ese ciclo estaba **roto** (404, #235), así que nunca se pudo probar. El **aviso del pie y la descarga del `.dmg`** con la app EMPAQUETADA (#231): en desarrollo el updater nunca reporta versión nueva (`app.isPackaged` false). El **pegar en la app viva** tras el arreglo de #227 — que ⌘V pegue en el campo de la llave y siga pegando dentro del lienzo. El aspecto de las **barras de desplazamiento en Windows** (#206), que desde macOS no se puede medir. El arreglo de arranque sin WebGPU (0.8.1) **en la máquina Windows que reportó el problema** (#202 · #203). El botón «Sugerir» del tab Spec con el motor **local** (#175). La barra de título propia sólo se ejercitó en **macOS**
...
| Metadatos del updater del release publicado | `npm run release:updater-check` (usa red) | verde — contra v0.8.6, los tres `latest*.yml` apuntan a un asset que existe y con el tamaño que declaran. Contra v0.8.5 este mismo comando reporta el 404 de Windows (#235): es la señal que faltaba para cazar un release ininstalable sin tener la máquina delante |
| Nombre del instalador sin espacios | `node scripts/repo-lint.mjs` (regla ARTIFACTNAME) | verde — un `artifactName` con espacios, o ausente (el default de electron-builder los trae), pone el gate en rojo: es la raíz del 404 de #235, porque GitHub sube los espacios como puntos y el yml los escribe con guiones |
...
| El agente ve el contrato de la caja | `npx vitest run src/lib/ai/__tests__/agent-retrieval.test.ts src/lib/ai/__tests__/agent-run.test.ts` | verde — `read_element` devuelve la descripción ENTERA, las propiedades y la spec (con las marcas «por aclarar»), y el digest de una vista marca `{spec,props,desc+}` en la caja que tiene más de lo que muestra. Antes el agente veía 90 caracteres de descripción por nodo y nada del contrato (#239) |
| La cita de una caja resuelve a su fragmento | `npx vitest run src/lib/__tests__/source-docs.test.ts` | verde — 25 pruebas: adjuntar reemplaza por nombre y recorta con aviso; resolver una cita tiene TRES desenlaces y ninguno inventa (sin cita de archivo nada · documento ausente se dice · presente devuelve las líneas con dos de contexto); una cita en prosa («PRD §3.2») sigue siendo texto, así los diagramas viejos no cambian (#240) |
| El documento NO se cuela al contexto | `npx vitest run src/lib/ai/__tests__/graph-toon.test.ts` | verde — `source_docs` está en `NOISE_KEYS`: el texto viaja con el proyecto pero nunca en el TOON. Un documento de 60 000 caracteres reventaría la ventana de 4 096 tokens del motor local; el agente lo lee por trozos con `read_source` |
