...
- **Versión publicada:** 0.8.5 — publicada el 2026-09-01 (`gh release edit v0.8.5 --draft=false`), con los 8 artefactos y como `latest` de la API. Trae #231: el aviso de actualización se fue de la barra al pie del sidebar, «Buscar actualizaciones…» entró al menú Ayuda, y en macOS la app baja el `.dmg` a Descargas en vez de abrir el navegador. La 0.8.4 (arreglo del pegar, #227) también quedó publicada; `v0.8.3` quedó en borrador y superada, `v0.7.0` también
- **Versión en el repo:** 0.8.5 — igual a la publicada (PR #233)
- **Cierres verificados el 2026-08-28:** #199 (E2E stdio 15/15 sobre `set_element_spec` · `get_element_spec` · `spec_to_markdown` · `review_specs` + reviewer sin bloqueantes), #185 (SC-005: con una spec de 20 historias/100 escenarios/100 requisitos, el costo por tecla de la parte pura —copia + `sanitizeSpec` + `isSpecEmpty` + `specToMarkdown`— dio p95 0.15 ms, muy bajo los 100 ms) y #184 (`tasks-registry.test.ts` 5/5; la lentitud del motor local en tareas `heavy` queda abajo como limitación conocida)
- **Sin verificar:** el **ciclo completo de actualización en Windows** (#208 · T9 · #217): con 0.8.4 ya publicada, falta instalar 0.8.2 ahí, pulsar «Actualizar» y ver 0.8.4 en el pie. El **aviso del pie y la descarga del `.dmg`** de #231 con la app EMPAQUETADA: en desarrollo el updater nunca reporta versión nueva (`app.isPackaged` false), así que 0.8.5 es la primera oportunidad de verlo. El **pegar en la app viva** tras el arreglo de #227 — que ⌘V pegue en el campo de la llave y siga pegando dentro del lienzo — se dedujo del mecanismo de aceleradores y quedó cubierto por una prueba del catálogo, pero nadie lo probó con la app abierta. El aspecto de las **barras de desplazamiento en Windows** (#206), que desde macOS no se puede medir. El arreglo de arranque sin WebGPU (0.8.1) **en la máquina Windows que reportó el problema** — se verificó por pruebas, typecheck y build, no ahí (#202 · #203). El botón «Sugerir» del tab Spec con el motor **local** — Gemma 4 arranca en WebGPU pero no devolvió borrador en ~7 min de espera, y el resto del tab funciona sin IA (#175). La barra de título propia sólo se ejercitó en **macOS**: en Windows y Linux los controles de ventana son los nativos del sistema (overlay), pero el arrastre y el botón de menú no los vio nadie
