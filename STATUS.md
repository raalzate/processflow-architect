...
- **Versión en el repo:** 0.8.3 — bump + notas en `docs/releases/0.8.3.md`, pendiente de PR/tag; existe para ser la primera versión entregada POR el botón (T9 · #217). El empaquetado lo hace `release-build.yml` y deja el release en **BORRADOR** a propósito: publicarlo es un paso humano
- **Sin verificar:** el **ciclo completo de actualización en Windows** (#208 · T9 · #217): con 0.8.2 publicada, falta etiquetar y publicar 0.8.3 y probarlo de punta a punta. El aspecto de las **barras de desplazamiento en Windows** (#206), que desde macOS no se puede medir. El arreglo de arranque sin WebGPU (0.8.1) **en la máquina Windows que reportó el problema** — se verificó por pruebas, typecheck y build, no ahí (#202 · #203). El botón «Sugerir» del tab Spec con el motor **local** — Gemma 4 arranca en WebGPU pero no devolvió borrador en ~7 min de espera, y el resto del tab funciona sin IA (#175). La barra de título propia sólo se ejercitó en **macOS**: en Windows y Linux los controles de ventana son los nativos del sistema (overlay), pero el arrastre y el botón de menú no los vio nadie
