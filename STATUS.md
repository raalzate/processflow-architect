...
- **Fecha del último gate completo:** 2026-08-28
- **Rama:** `main`
- **Veredicto:** VERDE (`npm run gate`)
- **Versión publicada:** 0.8.2 — publicada el 2026-08-28 (`gh release edit v0.8.2 --draft=false`), con los tres instaladores, `latest*.yml` y blockmaps adjuntos (verificado con `gh release view`). Es la **primera con el botón «Actualizar» y los metadatos del updater**: el botón ofrece la SIGUIENTE, no esta. `v0.8.0` y `v0.8.1` también quedaron publicadas; `v0.7.0` sigue en borrador, superada
- **Versión en el repo:** 0.8.3 — mergeada a `main` por el PR #222, etiquetada (`v0.8.3`) y empaquetada por `release-build.yml` en verde: el **BORRADOR** del release tiene los 8 artefactos (tres instaladores, `latest*.yml`, blockmaps). Publicarlo es el paso humano que falta (`gh release edit v0.8.3 --draft=false`); es la primera versión que llega POR el botón (T9 · #217)
...
- **Sin verificar:** el **ciclo completo de actualización en Windows** (#208 · T9 · #217): con el borrador de 0.8.3 listo, falta publicarlo y probarlo de punta a punta. El aspecto de las **barras de desplazamiento en Windows** (#206), que desde macOS no se puede medir. El arreglo de arranque sin WebGPU (0.8.1) **en la máquina Windows que reportó el problema** — se verificó por pruebas, typecheck y build, no ahí (#202 · #203). El botón «Sugerir» del tab Spec con el motor **local** — Gemma 4 arranca en WebGPU pero no devolvió borrador en ~7 min de espera, y el resto del tab funciona sin IA (#175). La barra de título propia sólo se ejercitó en **macOS**: en Windows y Linux los controles de ventana son los nativos del sistema (overlay), pero el arrastre y el botón de menú no los vio nadie
